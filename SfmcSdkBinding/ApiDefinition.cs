...
namespace SfmcSdkBinding {
	...
	interface MarketingCloudSDK {
		...
		MarketingCloudSDK SharedInstance { get; }

		// -(BOOL)sfmc_configure:(NSError * _Nullable * _Nullable)error;
		[Export("sfmc_configure:")]
		bool Configure([NullAllowed] out NSError error);
		...
		bool ConfigureWithURL([NullAllowed] NSUrl url, NSNumber configurationIndex, [NullAllowed] out NSError error);
		...
		bool Configure([NullAllowed] out NSError error, [NullAllowed] Action<bool, NSString, NSError> completionHandler);
		...
		bool ConfigureWithURL(NSUrl url, NSNumber configurationIndex, [NullAllowed] out NSError error, [NullAllowed] Action<bool, NSString, NSError> completionHandler);
	...
	interface MarketingCloudSDKURLHandlingDelegate {
		...
		void HandleURL(NSUrl url, string type);
	}
